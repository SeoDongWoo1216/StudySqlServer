...
-- 잘못된 필터링
select userID, sum(price * amount) as '전체 구매 금액'
from buytbl
--WHERE sum(price * amount) >= 1000
group by userID
having  sum(price * amount) >= 1000
order by sum(price * amount) desc;  -- order by는 집계함수가 나와도 됨.


-- rollup, cube
select num, groupName, sum(price * amount) as '구매 금액', -- 집계함수가 들어가면 무조건 그룹바이 필요
		GROUPING_ID(groupName, num)  -- 결과가 0이면 원래있던 데이터, 1이면 합계를위해 추가된 열이라고 보면됨
from buytbl
GROUP BY rollup(groupName, num);   -- 집계함수 외의 모든 컬럼을 적어줘야한다.
-- rollup을 통해 groupName끼리의 소합계, 모든 총합까지 다 조회할 수 있다.


-- userID, groupName으로 cube 다차원 합계(ROLLUP이 더 쉬우니까 롤업만 쓰자)
select userID, sum(price * amount) as '구매 금액'
from buytbl
GROUP BY cube(userID);   

select groupName, sum(price * amount) as '구매 금액'
from buytbl
